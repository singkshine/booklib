...
CHAIN_BOOL new_in_book(void *books){
    ...
    if(head==NULL)
    {
        ...
        if(serch_book(book)==NULL){
            ...
            int i=0;
            if(type_input(serch_book(book)->sum)==STR)
            strcpy(serch_book(book)->sum,"0\0");
...
CHAIN_BOOL serch_num(char *p){
    if(type_input(p)==STR||head==NULL)
    return SYSTEM_FALSE;
    BOOK*temp=head;
    while(temp!=NULL){
        if(check(temp->sum,p)==SYSTEM_RIGHT){
            stack_push(temp);
        }
        temp=temp->NEXT;
    }
